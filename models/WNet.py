...
class WNetSIM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(WNetSIM, self).__init__()
        # edge net
        self.edge_cont1 = double_conv(in_channels, 64)
        self.edge_maxpool1 = nn.MaxPool2d(2)
        self.edge_cont2 = double_conv(64, 128)
    ...
    def forward(self, x):
        ...
        y = self.edge_up1(y4)
        y = self.edge_expa1(copy_and_crop(y3, y))
        y = self.edge_up2(y)
        y = self.edge_expa2(copy_and_crop(y2, y))
        y = self.edge_up3(y)
        y = self.edge_expa3(copy_and_crop(y1, y))
        ...
        return self.seg_output(x), self.edge_output(y)
